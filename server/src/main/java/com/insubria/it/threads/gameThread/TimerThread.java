 ...
 * timer and review timer). This class follows the observable/observer pattwern.
 * This thread is the observable object and the clients that plays the game this
 * thread represents are the observers. This class extends the Thread interface
 * to let the instances of this class to be threads.
 */
public class TimerThread extends Thread {
    ...
    /**
     * The reference to the GameThread instance responsible for the game the timer
     * thread is counting for
     */
     ...
     * @param gameResponsible     - The reference of the GameThread that started the
     *                            TimerThread thread
     * @param gameClientObservers - The list of clients the timer thread communicate
     *                            the available time.
     */
    ...
    /**
     * This method perform the count down and sends to the clients the remaining
     * time available.
     * 
    ...
    /**
     * When the thread starts it count the 3 minutes. After that, if the scope is
     * "isPlaying" the thread call the GameThread method to stop the clients from
     * playing. If the scope is different it means that the review time is ended and
     * a new session needs to be played (if nobody reached 50 score)
     */
    public void run() {
        try {
